...
export class NewsItem extends Component {
    ...
    render() {
     let {title,image,desc,newsUrl,publishedAt,author}=this.props;
        return (
            <div>
                <div className="card" style={{width: "21rem"}}>
                    ...
                    <div className="card-body">
                        ...
                        <p class="card-text"><small class="text-muted">By {author?author:"Unknown"} at {new Date(publishedAt).toGMTString()}</small></p>
                        <a href={newsUrl} rel="noopener noreferrer" target='_blank' className="btn btn-sm btn-primary">more</a>
                    </div>
                </div>
            </div>
        )
